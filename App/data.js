      ...
      .then(data => {
        fetch(`https://anapioficeandfire.com/api/characters/${element}`)
          ...
          .then(ret => {
            ...
            const killButton = document.querySelector('#kill-random-lord-button');
...
function killLord() {
  ...
  let randomSelectionArray = gameofthrones[randomNum].swornMembers;
  let randomSworn = Math.floor(Math.random() * parseInt(randomSelectionArray.length));
  fetch(`${randomSelectionArray[randomSworn]}`)
    .then(response => response.json())
    .then(result => {
      const replaced = document.querySelectorAll('.got-house__current-lord');
        replaced[randomNum].innerHTML = result.name;
    });  
}